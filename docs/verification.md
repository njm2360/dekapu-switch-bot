...
| 実機制御ログ   | `logs/run_<日時>.csv`                                       | `ControlLog`(recording.py)形式。sim の記録と同じ列                                                     |
...
```python
from vrc_autopilot.control.controller import ControlTuning, nav_controllers
...
gains = ControlTuning()                       # フィールド指定で部分上書き
...
HUD は4軸バー(turn/pitch/fwd/strafe)+フェーズ(色分け)+誤差+アクティブ軸の PID 内訳+実ヨーレート/実速度。バーの赤い目盛りが不感帯オンセット(turn ±0.50、他 ±0.10)で、バーが目盛りに届いていない間は指令が効いていない。実機ログ(`logs/run_*.csv`)もそのまま読める。
