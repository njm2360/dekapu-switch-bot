...
- 全ループはヘッドレス実行可: `SimulatedVRChat`(app/sysid/sim_plant.py)が reader/look/move の3役、`SimClock` を clock に渡すと仮想時間で高速に回る。呼び出しパターンは tests/test_sysid.py の `test_all_control_loops_run_against_sim` が実例。
