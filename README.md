...
グリッド定数(オフセット/ブロック)は [pose_hud/spec.py](pose_hud/spec.py) の確定値。変更はシェーダーと要一致。
...
制御は**連続追従**(経由点で止まらない)+ 経路は**壁に遮られない直線で間引く**(ジグザグ除去)+
実 dt のフィードバック。移動中(nav)は穏やかな yaw ゲイン、到着後(face)は yaw/pitch を `--face-tol` 未満へ収束。
...
各機能はサブモジュールから直接 import する(`pose_hud` 直下に再エクスポートは無い)。
...
非Windows/テストでは `pose_hud.capture.ArrayFrameSource(frame)` を `PoseReader` に注入する。
