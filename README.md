...
- オフライン検証の組み方(sim・指標・落とし穴): [docs/verification.md](docs/verification.md)
...
前提(ライブ実行時): VRChatをネイティブ解像度・ウィンドウ表示で起動し、
...
読み続けて表示する(`--stats` で fps・成功率も)。グリッド定数は
...
「ボタンを画面中央に捉えて SPACE」で視線レイを記録し、2本以上の最小二乗交点=ボタン座標を表示する。
別方向から狙うほど精度が出る(平行に近いと警告)。

### `patrol-buttons` — 壁を避けてボタン巡回(OSC + PID)

部屋マップ上で指定座標のボタンへ A* で壁を避けて移動し、正対まで行う(クリックは未実装)。
...
- 制御の構成: 移動中(nav)は経路の少し先(`--nav-lookahead`)を狙う carrot 方式で連続追従し、
  ...
  ゲインの既定値・安定範囲・調整手順は [docs/gain-tuning.md](docs/gain-tuning.md)
- `--look mouse` で視点を DirectInput 相対マウスに切替可(要 pydirectinput)。不感帯が無いので
  deadzone は 0 にし、ゲインは別物として再調整する
- 制御ログ: `logs/patrol_<日時>.csv` に毎フレームの姿勢・誤差・PID内訳・指令を書き出す
  (チューニングと原因調査用。`sim-video` でそのまま再生できる)

### `sim-video` — 制御ログを動画で再生

フレームCSV(実機の `logs/patrol_*.csv`またはsimの記録)を、一人称3D(レイキャスト)+
上面2D地図の並置mp4にする。経路追従・ぎこちなさの目視確認用。要ffmpeg。
...
`/input/LookVertical`。Buttons(int 0/1): `/input/Jump` ほか。
